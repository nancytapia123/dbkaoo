    ...
    success: function (respuesta){
      console.log("data: ", respuesta);
        if(respuesta){
          ...
          document.getElementById("frm_reset").reset();
        }else{
...
function BtnEditarAjax(id) {
  
  $('#idAux').val(id);
  
  var url = $('#editAjax').attr('url');
  var table = $('#editAjax').attr('table');
  var titulo_modal = $('#editAjax').attr('title_modal');
 
  $.ajax({
      url:url,
      type: "POST",
      data: {"action":"edit",id:id,"table":table},
      async: false,
      dataType: "json",
      success: function (respuesta){
...
function BtnElimanarAjax(id){
  ...
  var table = $('#removeAjax').attr('table');
