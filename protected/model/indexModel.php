...
class indexModel
{
    ...
    public function validarAcceso($usuario, $pass, $id = null)
    {
        ...
        if (is_null($id)) {
            $ss = "SELECT a.*, count(*)as nr, b.rol FROM usuario as a INNER JOIN rol as b ON a.rol_id=b.id WHERE a.correo = '" . $usuario . "' AND a.password=MD5('" . $pass . "') AND status_id = 1 GROUP BY id";
        } else {
            $ss = "SELECT a.*, count(*)as nr, b.rol FROM usuario as a INNER JOIN rol as b ON a.rol_id=b.id WHERE a.id={$id} GROUP BY id";
        }
        //echo $ss."<hr>";
        //exit();
        $ultimas = $this->db->prepare($ss);
        ...
        if ($res->nr == 1) {
           ...
           */
            ...
            setcookie('Nombre', $res->usuario, time() + (86400 * 30), '/', $_SERVER["SERVER_NAME"]);
            //var_dump($_COOKIE);
            //exit();
    ...
    public static function sendMail($correo, $name, $asunto, $mensaje, $opc = 0)
    {
        ...
        //echo 1;
        $mail = new PHPMailer();
        //$mensaje="<img  alt=\"KAO Hoteles\" src=\"http://admin.notify.com.mx/assets/image-resources/logo.png\"><br><br>".$mensaje;
